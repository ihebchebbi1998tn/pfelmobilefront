...
import OrderWarnningMessage from './OrderWarnningMessage'
...
const [openTutorial, setOpenTutorial] = useState(() => {
 ...
 const ordersByStatus = STATUSES.reduce((acc, status) => {
    acc[status] = orders.filter((i) => i.status === status)
    return acc
  }, {})
 ...
 const onDragEnd = async (result) => {
    ...
    const item = orders.find((i) => i.id.toString() === draggableId)
    if (!item) return
    ...
    if (!statusOrders[cs]?.includes(ns)) return
    if(cs==='InProgress' && ns==='Delivered' && !item.isPaid && item.paymentMethod!=='Cash'){
    ...
    <Box>
       ...
       {showWarningMessage && (
        ...
        {openForm && (
         ...
         <Card title={dictionary.CustomersOrders} showHover={false}>
        ...
        {(orders!=null && orders.length>0 && hasAnyOrder) ?
        (
            <Box>
                {orders.length>0 ? (
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        {items.map((order, index) => {
                                            const client =getUserInfos(order.userId)
                                        return(
                                            <Draggable
                                            draggableId={order.id.toString()}
                                            index={index}
                                            key={order.id}
                                            isDragDisabled={['Cancelled','Refused'].includes(order.status)}
                                        >
                                            {(provided, snapshot) => (
                                            <Paper
                                            onClick={() => HandleOpenModalDetailsInstallation(order)}
                                                ref={provided.innerRef}
                                                ...
                                                cursor: ['Cancelled','Refused'].includes(order.status) ? 'not-allowed' : 'grab',
                                                userSelect: 'none', transition:'box-shadow 0.2s ease',
                                                border: `1px solid ${isDark ? '#ffffff' : '#9da7b7'}`,
                                                '&:hover': {
                                                    transform:['Cancelled','Refused'].includes(order.status) ? 'none' : 'scale(1.08)'
                                                    },
                                                }}
                                            >
