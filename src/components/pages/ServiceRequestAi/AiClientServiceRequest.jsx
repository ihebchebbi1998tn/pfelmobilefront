...
const AiClientServiceRequest = () => {
 ...
 const [openPaymentMethode, setOpenPaymentMethode] = useState(false)
 ...
 const handleSearchChange = (searchTerm) => {
  ...
  const HandleOpenModalMethodePayment = (row) =>{
    setSelectedRow(row)
    setOpenPaymentMethode(true)
 }
     ...
     >
       ...
       >
           ...
           <Box>
             ...
             {agent!=null &&
                ...
                <Typography variant="body1" sx={{mb:1}}>
                    {truncateText(service?.recap)}
                     {service?.recap.length > 40 && (
                  <Typography
                    component="span"
                    sx={{
                      cursor: 'pointer',
         ...
         <Box display="flex" justifyContent="space-between" p={2} pt={0}>
            ...
            {(service?.status==='Pending' && !service?.isPaid && service?.paymentMethod===null && user.organization.onlinePaymentSolutions != null)&&
              <Button
                    variant={ 'outlined blinkingZoom' }
                    onClick={(e) => {
                            e.stopPropagation()
                            HandleOpenModalMethodePayment(service)
                        }}
                >
                  {' '}
                <WarningIcon sx={{ 
  ...
  const fetchServicesData = async (page,searchTerm) => {
     try {
       ...
       if (data) {
        ...
        if( data.items.length > 0 && user.organization.onlinePaymentSolutions === null) {
          for(let i=0; i < data.items.length; i++) {
            await ServiceAiRequest.addPaymentMethode(data.items[i].id, 'Cash')
          }
...
export default AiClientServiceRequest